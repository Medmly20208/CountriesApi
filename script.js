...
function changeFunc() {
  ...
  while (i < BoxOfCountries.children.length) {
    if (
      BoxOfCountries.children[i].children[1].children[1].textContent.includes(
  ...
  for (let j = 0; j < 6; j++) {
    ...
    dvElement.onclick = function () {
      ...
      localStorage.setItem(
        "topLevelDomain",
        jsonresponse[ar[j]].topLevelDomain
      );
      localStorage.setItem(
        "currencies",
        jsonresponse[ar[j]].currencies[
          Object.keys(jsonresponse[ar[j]].currencies)
        ].name
      );
      localStorage.setItem(
        "languages",
        Object.values(
          jsonresponse[ar[j]].languages.map(function (ele) {
            return ele.name;
          })
        )
      );
...
searchButton.onclick = async function () {
  let response1 = await fetch(
    `https://restcountries.com/v3.1/name/${
      inputCountry.value.toLowerCase() == "israel"
        ? "palestine"
        : inputCountry.value.toLowerCase() == "western sahara"
        ? "morocco"
...
`;
  ...
  dvElement.onclick = function () {
    ...
    localStorage.setItem(
      "NativeNme",
      jsonresponse1[0].name.nativeName[
        Object.keys(jsonresponse1[0].name.nativeName)[0]
      ].official
    );
    ...
    localStorage.setItem(
      "currencies",
      jsonresponse1[0].currencies[Object.keys(jsonresponse1[0].currencies)].name
    );
    localStorage.setItem(
      "languages",
      Object.values(jsonresponse1[0].languages)
    );
